...
| `EXPO_ADB_TIMEOUT`                   | **number**  | Duration in milliseconds to wait for ADB commands that are expected to return promptly, such as starting the ADB server and listing devices, before assuming the ADB server is unresponsive. Set to `0` to wait indefinitely. Defaults to `15000`.                                                                                                                                                                              |
