...
- Fix the embedded manifest pairing assets with the wrong `packagerHash` when an asset ships scales that iOS does not support (`@1.5x`/`@4x`), which made those images resolve to an empty URI and render blank in release builds. ([#XXXXX](https://github.com/expo/expo/pull/XXXXX) by [@ahmdshrif](https://github.com/ahmdshrif))
