...
class Query(
  ...
  suspend fun exeWithDetails(): List<AssetInfo> = withContext(Dispatchers.IO) {
    val queryExecutor = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
      QueryModernExecutor(clauses, args, orderBy, limit, offset)
    } else {
      QueryLegacyExecutor(clauses, args, orderBy, limit, offset)
    ...
    val mediaStoreItems = queryExecutor.exe(AssetMediaStoreItem.projection, contentResolver).use {
      ensureActive()
    ...
    mediaStoreItems.map { mediaStoreItem ->
      ensureActive()
      assetMapper.toDto(mediaStoreItem)
    }
  }
}
